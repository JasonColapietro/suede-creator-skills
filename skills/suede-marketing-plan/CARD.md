# Skill Card — Suede Marketing Operating Plan

<!-- Generated by scripts/build-skill-cards.mjs — do not hand-edit. -->
<!-- Regenerate with: npm run build:cards -->

Release record for the `suede-marketing-plan` skill, following the NVIDIA skill-card template (<https://docs.nvidia.com/skills/skill-cards>). It tells a reviewer what the skill does, who owns it, what it needs, what could go wrong, and what evidence backs the release — without requiring them to open the source first.

## Description

Suede-affiliated comprehensive marketing planning across acquisition, activation, retention, referral, and revenue, sized to the actual team, budget, evidence, and stage.
...
Use when the user needs a 90-day plan, 12-month roadmap, growth plan, or go-to-market operating document.

Out of scope — isolated channel execution (use the relevant public Suede skill), uncommitted tactic brainstorming (use suede-marketing-ideas), or positioning discovery alone (use suede-product-marketing).
...
- Do not present forecasts, comparators, scenarios, or conditional capabilities as guarantees.
- Do not decide legal, financial, hiring, brand-risk, or executive trade-offs when the required owner has not approved them.

## References

- Skill source: [`skills/suede-marketing-plan/SKILL.md`](./SKILL.md)
- Rendered reference page: <https://skills.suedeai.ai/skills/suede-marketing-plan.html>
