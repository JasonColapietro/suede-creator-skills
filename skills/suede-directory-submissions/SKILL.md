...
description: "Work the directory layer deliberately: which listings carry dofollow weight, submission order, launch-day sequencing, and verifying the backlink landed. Use when the user is submitting a product to startup, SaaS, AI, or MCP directories, or wants backlinks from listings. Also use when the user mentions 'directory submissions,' 'submit to directories,' 'backlinks from directories,' 'list my product,' 'submit to Product Hunt,' 'BetaList,' 'TAAFT,' 'Futurepedia,' 'G2 listing,' 'Capterra listing,' 'AlternativeTo,' 'SaaSHub,' 'AI directories,' 'MCP registry,' 'agent directory,' 'dofollow backlinks,' 'launch directories,' or 'directory tracker.' Use this whenever someone is planning the directory layer of a product launch or an ongoing backlink campaign. For the broader launch moment, see suede-launch-packaging. For programmatic SEO pages that should live behind these backlinks, see suede-programmatic-seo. For AI citation optimization, see suede-seo-audit."
metadata:
  version: 2.0.0
---
