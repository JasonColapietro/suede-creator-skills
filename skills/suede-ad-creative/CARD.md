...
Structured Markdown returned in the agent's response, shaped by the output contracts defined in the skill body: "Output Formats", "Standard Output", "Bulk CSV Output", "Static Batch Output (Mode 3)". Any externally visible action — publishing, posting, sending, submitting, deploying — happens only with the user's explicit authorization; by default the skill hands its work back in the response.
