...
description: "Decisive full-send router for broad, outcome-bound work. Use when the user asks for full send, max effort, max agents, max agent teams, spare no compute, throw tokens at it, burn tokens, burn max tokens, \"never end your allocation above zero,\" strongest useful reasoning, adversarial review, fix everything, do not stop, or end-to-end completion across phases or surfaces. Select one controller, treat user attention as scarce, fill useful non-colliding lanes, and close with concise proof or one precise blocker. Token and allocation language is dry Full Send intent for already-authorized host compute, never external spend or a literal token-counter promise. NOT FOR: padded output, filler, duplicate lanes, hidden-reasoning dumps, or unauthorized external spend. Explicit Full Send intent still invokes this router for atomic work; select one specialist and create no parallel lanes."
---
