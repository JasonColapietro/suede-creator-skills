...
repo total 1,053,496 bytes. Loading all of them into every conversation would
crowd out the thing you actually came to do.

They are not all loaded. Only the frontmatter descriptions stay resident, and
all 72 descriptions together come to 42,755 bytes. That is roughly a
...
when the skill is not running. In this repo the 72 files total 1,053,496 bytes;
the 72 descriptions together are 42,755. The agent holds the small number and
